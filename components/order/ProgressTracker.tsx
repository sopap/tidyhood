  ...
  return (
    <>
      ...
      <div className="md:hidden">
        ...
        >
          <ol className="flex items-center px-4 py-3 min-w-max" aria-label="Order progress">
            {ORDER_STEPS.map((s, i) => {
              const done = i <= idx;
              return (
                <React.Fragment key={s.key}>
                  <li className="relative flex flex-col items-center gap-1">
                    ...
                    {i === idx && (
                      <span className="text-[10px] font-medium text-blue-600 text-center whitespace-nowrap">Current</span>
                    )}
                  </li>
                  {i < ORDER_STEPS.length - 1 && (
                    <div className="h-px w-8 bg-gray-300 mx-2 flex-shrink-0" aria-hidden="true" />
                  )}
                </React.Fragment>
              );
            })}
      ...
      <div className="hidden md:block">
        <ol className="relative ml-4 border-l-2 border-gray-200 pl-8" aria-label="Order progress">
          {ORDER_STEPS.map((s, i) => {
            const done = i <= idx;
            return (
              <li key={s.key} className="mb-8 last:mb-0 relative">
                <span 
                  className={`absolute -left-[2.15rem] grid h-7 w-7 place-items-center rounded-full text-sm ring-4 ring-white transition-all ${
                    done ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-600'
