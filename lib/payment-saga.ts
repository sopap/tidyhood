...
export class PaymentAuthorizationSaga {
  ...
  private async savePaymentMethod(
    ...
    const customerId = await this.getOrCreateStripeCustomer(params.user_id);
    
    // Check if payment method needs to be re-attached to this customer
    try {
      const paymentMethod = await stripe.paymentMethods.retrieve(params.payment_method_id);
      ...
      // If retrieval fails, payment method might be newly created, continue
      logger.warn({
