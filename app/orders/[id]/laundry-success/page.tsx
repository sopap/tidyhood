...
export default function LaundrySuccessPage() {
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-4">
                ...
                <div>
                  <p className="font-medium text-gray-900">Delivery Confirmed</p>
                  <p className="text-sm text-gray-600">
                    {order.order_details.preferredDeliveryDate ? (
                      <>
                        Scheduled for {new Date(order.order_details.preferredDeliveryDate + 'T12:00:00').toLocaleDateString('en-US', {
                          timeZone: 'America/New_York',
                          weekday: 'long',
                          month: 'long',
                          day: 'numeric'
                        })}
                        {order.delivery_slot && ` at ${formatTime(order.delivery_slot.slot_start)}`}
                      </>
                    ) : (
                      'Delivery date to be coordinated'
                    )}
                  </p>
                </div>
              </div>
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium text-gray-900 text-right">
                  {getServiceLabel()}
                  {order.order_details.rushService && (
              ...
              {order.order_details.preferredDeliveryDate && (
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-gray-900 text-right">
                    {new Date(order.order_details.preferredDeliveryDate + 'T12:00:00').toLocaleDateString('en-US', {
                      timeZone: 'America/New_York',
                      month: 'short',
                      day: 'numeric'
                    })}
                    {order.delivery_slot && ` · ${formatTime(order.delivery_slot.slot_start)} - ${formatTime(order.delivery_slot.slot_end)}`}
                    {!order.delivery_slot && ' · Time TBD'}
                  </span>
                </div>
              )}
              <div className="flex justify-between">
